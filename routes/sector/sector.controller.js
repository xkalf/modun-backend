...
async function getSector (req, res) {
  try {
    ...
    if (sectors.length > 0) return res.status(200).json(sectors)
    else return res.status(400).json('sector can not found')
  } catch (err) {
...
async function createSector (req, res) {
  try {
    ...
    if (!newSector) return res.status(400).json('Can not create sector')
...
async function updateSector (req, res) {
  try {
    ...
    if (!updatedSector) return res.status(500).json("Can't update")
